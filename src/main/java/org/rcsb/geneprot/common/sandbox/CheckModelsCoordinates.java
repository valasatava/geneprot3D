...
public class CheckModelsCoordinates {
    ...
    public static void main(String[] args) {
        ...
        List<Row> modelsData = HomologyModelsProvider.getAsDataFrame(DataLocationProvider.getHumanHomologyModelsLocation()).collectAsList();
        List<String> templates = modelsData.stream().map(t -> t.getString(12) + "_" + t.getInt(3) + "_" + t.getInt(13)).collect(Collectors.toList());
        for (String t : templates) {
            File file = new File(DataLocationProvider.getHumanHomologyCoordinatesLocation() + t + ".pdb");
            if (!file.exists()) {
